...
def test_list_playlists(mock_listdir):
    ...
    playlists = list_playlists('/fake/dir', pattern)
...
def test_delete_old_playlists(mock_remove):
    ...
    delete_old_playlists('/fake/dir', playlists, 1)
    mock_remove.assert_called_once_with('/fake/dir/20230101120000playlist.wpl')

@patch('time.strftime')
def test_generate_playlist_filename_with_output(mock_strftime):
    filename = generate_playlist_filename('/fake/dir', 'output.wpl', 'wpl')
    assert filename == '/fake/dir/output.wpl'

@patch('time.strftime')
def test_generate_playlist_filename_without_output(mock_strftime):
    mock_strftime.return_value = '20230101120000'
    filename = generate_playlist_filename('/fake/dir', None, 'wpl')
    assert filename == '/fake/dir/20230101120000playlist.wpl'

@patch('builtins.open', new_callable=mock_open)
def test_write_playlist(mock_open):
    filename = '/fake/dir/playlist.wpl'
    format = 'wpl'
    mediaList = []
    randomList = []
